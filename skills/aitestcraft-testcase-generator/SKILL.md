---
name: aitestcraft-testcase-generator
description: >
  生成软件测试用例。支持按模块分组输出测试用例，可参考知识库上下文进行用例设计。
version: 4.1
author: leizi
---
...
# TASK
根据输入的测试点列表生成可执行测试用例。输入可能包含多个模块的测试点，需按模块分组输出。如果输入包含知识库上下文，请参考历史需求知识点进行用例设计，确保用例的完整性和一致性。
...
2. 根据知识库中的已有知识点，补充完善测试用例覆盖范围
3. 确保新需求的测试用例与已有知识保持一致的用例风格和粒度
